...
namespace System.Web.Services.Protocols {
    ...
    internal class WebServiceHandler {
        ...
        async System.Threading.Tasks.Task InvokeAsync()
        {
            PrepareContext();
            protocol.CreateServerInstance();
            ...
            try
            {
                TraceMethod caller = Tracing.On ? new TraceMethod(this, "InvokeAsync") : null;
                TraceMethod userMethod = Tracing.On ? new TraceMethod(protocol.Target, protocol.MethodInfo.Name, this.parameters) : null;
                if (Tracing.On) Tracing.Enter(protocol.MethodInfo.ToString(), caller, userMethod);
                object[] returnValues = await protocol.MethodInfo.InvokeAsync(protocol.Target, this.parameters);
                if (Tracing.On) Tracing.Exit(protocol.MethodInfo.ToString(), caller);
                WriteReturns(returnValues);
            }
            catch (Exception e)
        ...
        protected async System.Threading.Tasks.Task CoreProcessRequestAsync()
        {
            try
            {
        ...
        private async Task OneWayInvokeAsync()
        {
            HttpContext oldContext = null;
            if (protocol.Context != null)
                oldContext = SwitchContext(protocol.Context);
    ...
    internal class TaskSessionlessHandler : SyncSessionlessHandler, IHttpTaskHandler
    {

        internal TaskSessionlessHandler(ServerProtocol protocol) : base(protocol) { }
        ...
        {
            return this.CoreProcessRequestAsync();
        }
    }
