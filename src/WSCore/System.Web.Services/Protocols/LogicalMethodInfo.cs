...
namespace System.Web.Services.Protocols {
    ...
    public sealed class LogicalMethodInfo {
        ...
        internal LogicalMethodInfo(MethodInfo methodInfo, WebMethod webMethod) {
            ...
            if (!isVoid && typeof(Task).IsAssignableFrom(retType))
            {
                ...
                if (!retType.IsGenericType)
                {
                    ...
                    this.retType = retType.GenericTypeArguments[0];
            }
        ...
        [PermissionSet(SecurityAction.LinkDemand, Name = "FullTrust")]
        public async Task<object[]> InvokeAsync(object target, object[] values)
        {
            if (outParams.Length > 0)
            {
                object[] newValues = new object[parameters.Length];
            ...
            object returnValue = methodInfo.Invoke(target, values);
            var taskReturn = (Task)returnValue;
            var isVoid = this.isVoid;

            if (methodInfo.ReturnType.IsGenericType)
            {
                var task = TaskConverter.Convert(taskReturn);
        ...
        class TaskConverter
        {
            public static Task<Object> Convert(Task task)
            {
                var returnType = task.GetType();

                returnType = returnType.GenericTypeArguments[0];
                var converterType = typeof(ConverterImpl<>).MakeGenericType(returnType);
            ...
            {
                Task<object> Convert(Task task);
            }

            class ConverterImpl<T> : IConverter
            {
                public Task<object> Convert(Task<T> task)
                {
                ...
                Task<object> IConverter.Convert(Task task)
                {
                    return this.Convert((Task<T>)task);
                }
            }
        }
